...
public class CreateNTLFabric implements ModInitializer {
    @Override
    public void onInitialize() {
		...
		CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
			NTLCommands.register(dispatcher, environment.includeDedicated));
    }
	...
	private static <L, R> void registerConfigEvents() {
		...
		try {
			Event<L> loadingEvent = (Event<L>) modConfigEventsClass.getMethod("loading", String.class).invoke(null, CreateNTL.ID);
			L loading = createHandlerProxy(NTLConfigs::onLoad, modConfigEventsClass, "Loading", "onModConfigLoading");
		...
		try {
			Event<R> reloadingEvent = (Event<R>) modConfigEventsClass.getMethod("reloading", String.class).invoke(null, CreateNTL.ID);
			R reloading = createHandlerProxy(NTLConfigs::onReload, modConfigEventsClass, "Reloading", "onModConfigReloading");
